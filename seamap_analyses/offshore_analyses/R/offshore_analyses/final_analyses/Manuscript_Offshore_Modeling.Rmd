...
season_rasters <- seamap_west %>% 
  ...
  imap(function(x, y) {
    ...
    # stars::write_stars(
    #   obj = catch_ras, 
    #   dsn = here("seamap_analyses", str_c(y,"_total_catch.tif"))
    # )
    #
    # # CPUE
    #
    # stars::write_stars(
    #   obj = cpue_ras, 
    #   dsn = here("seamap_analyses", str_c(y, "_cpue_hectares.tif"))
    # )
    #
    # # Variance
    #
    # stars::write_stars(
    #   obj = variance_ras, 
    #   dsn = here("seamap_analyses", str_c(y,"_cpue_var_hectares.tif"))
    # )
    #
    # # Effort
    #
    # stars::write_stars(
    #   obj = effort_ras, 
    #   dsn = here("seamap_analyses", str_c(y, "_station_effort.tif"))
    # )
...
ggplot() +
  geom_tile(data = season_rasters$Summer$agg_data, 
            aes(x = lon_bin, y = lat_bin, fill = cpue_variance), 
            color = "white", size = 0.05) +
  geom_sf(data = usmap) +
  geom_sf(data = mex) +
  scale_fill_distiller(palette = "Spectral", na.value = "transparent") +
  ...
  scale_y_continuous() +
  labs(x = NULL, y = NULL) +
  guides(fill = guide_colorbar(title = "CPUE Variance - Summer",
                               title.hjust = 0.5, 
                               title.position = "top",
                               barwidth = 10)) +
  theme(legend.position = "bottom", 
...
# raster::plot(bathy_test, main = "Depths <= 60 fathoms")
# 
# 
# #Save the raster and the hull for Zack - local because dropbox is full
# st_write(summer_hull_sf, dsn = here("seamap_analyses/summer_analysis_hull.shp"))
# raster::writeRaster(bathy_test, 
#                     filename = here("seamap_analyses/summer_analysis_60fm_hull.tif"))

# #Load the pre-processed forms
# summer_hull_sf <- st_read(here("seamap_analyses/summer_analysis_hull.shp"))
# bathy_test <- raster::raster(here("seamap_analyses/summer_analysis_60fm_hull.tif"))
# 
# 
...
# raster_area_km <- round(raster_area, digits = 1)
# raster_area_hc <- round(raster_area * 100, digits = 1)

# Pre-processing determined raster area to be
raster_area_km <- 122297.8 #square kilometers
raster_area_hc <- 12229780

# Print area of Study Area according to raster object
print(paste("Area of Analysis Data Study Area (raster):", raster_area_km, " km2"))

print(paste("Area of Analysis Data Study Area (raster):", raster_area_hc, " Hectares"))
