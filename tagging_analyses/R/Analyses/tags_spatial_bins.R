...
#Data
tagging <- read_csv(str_c(db_path, "/processed_data", "/tags_clean_final.csv"))
recaps <- read_csv(str_c(db_path, "/processed_data", "/tags_recaps_clean_final.csv"))
...
#Write that out to make maps in Q
tag_bins <- tagging %>% 
  mutate(long_decdeg = round(long_decdeg, 2),
         lat_decdeg  = round(lat_decdeg, 2)) %>%
  group_by(long_decdeg, lat_decdeg) %>% 
  summarise(n_tagged = n())
write_csv(tag_bins, path = str_c(db_path, "/final_report_datasets", "/tagging_bins_2digits.csv"), col_names = TRUE)

#recapture bins also
recap_bins <- recaps %>% 
  mutate(long_decdeg = round(rlong, 2),
         lat_decdeg  = round(rlat, 2)) %>%
  group_by(long_decdeg, lat_decdeg) %>% 
  summarise(n_recaps = n()) 
write_csv(recap_bins, path = str_c(db_path, "/final_report_datasets", "/recaps_bins_2digits.csv"), col_names = TRUE)
